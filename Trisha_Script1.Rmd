...
# subset the test set in a similar manner (so the first 17 columns), and then center and standardize the way we did above, then test the performance of both models (so the test set thats standardized on g1a and the original test set on g1a_full). if the two perform similarly, just go with the non-stanadraized
...
pred1<-format(round(predict(g1a,test1)))
conf <- confusionMatrix(pred1, as.factor(test$class))

```

```{r}
pred2<-format(round(predict(g2,test1)))
conf <- confusionMatrix(pred2, as.factor(test$class))

print(confusionMatrix(pred1, test1))
print(caret::confusionMatrix(pred1, y_test)$class)
```
...
full <- glm(class ~ air_time + paper_time + disp_index + max_x_extension + max_y_extension + num_of_pendown, data = Mdata, family=binomial(link="logit"))
